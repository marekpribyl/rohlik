...
public interface OrdersApi {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successful operation", 
                    content = @Content(schema = @Schema(implementation = OrderRecord.class))),
            @ApiResponse(responseCode = "404", description = "Order not found")
    })
    @GetMapping("/{id}")
    Mono<ResponseEntity<OrderRecord>> getOrderById(
    ...
    Mono<ResponseEntity<OrderRecord>> createOrder(
    ...
    @PostMapping("/{id}/pay")
    Mono<ResponseEntity<OrderRecord>> payOrder(
    ...
    Mono<ResponseEntity<OrderRecord>> cancelOrder(
